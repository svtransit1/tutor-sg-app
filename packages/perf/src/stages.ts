...
interface StageProfile { name: string; description: string; medianMs: number; scatter: number; }
...
export function createSimulatedStage(p: StageProfile, o?: { fastMode?: boolean }): PipelineStage<string, string> {
  return { name: p.name, description: p.description, async execute(input: string): Promise<string> {
    const d = sampleLogNormal(p.medianMs, p.scatter);
    if (!o?.fastMode) await busyWait(d);
    return `${input} → [${p.name}]`;
  }};
}
export function createSimulatedPipeline(t: 'high'|'mid'='high', o?:{fastMode?:boolean}): PipelineStage<string, string>[] {
  return (t==='high'?HIGH:MID).map(p=>createSimulatedStage(p,o));
}
function sampleLogNormal(m:number,s:number):number{if(s<=0)return m;const mu=Math.log(m),u1=Math.random(),u2=Math.random(),z=Math.sqrt(-2*Math.log(u1))*Math.cos(2*Math.PI*u2);return Math.max(1,Math.exp(mu+s*z))}
function busyWait(ms:number):Promise<void>{return new Promise(r=>setTimeout(r,ms))}
