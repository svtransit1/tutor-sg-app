...
export class ModelDownloadManager {
  ...
  async stop(): Promise<void> {
    ...
    for (const [, resolve] of this.cellularResolvers) {
      resolve();
    }
  ...
  async startAll(): Promise<void> {
    for (const modelId of this.queue) {
      ...
      // If the item didn't complete (e.g. paused, failed), stop the queue
      // so the caller can decide whether to resume/cancel.
