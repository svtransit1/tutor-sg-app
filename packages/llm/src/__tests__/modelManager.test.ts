...
function makeConfig(
  overrides: Partial<ModelDownloadManagerConfig> = {},
): ModelDownloadManagerConfig {
...
describe('ModelDownloadManager', () => {
  ...
  describe('pause and resume', () => {
    it('should allow pausing a running download', async () => {
      ...
      // startAll has returned — resume starts a new download
      manager.resume('slow-model');

      // Wait for the resumed download to complete by polling
      for (let i = 0; i < 20; i++) {
  ...
  describe('sequential download queue', () => {
    it('should download all items sequentially', async () => {
      ...
      const { adapter, fs } = makeAsyncAdapter(1);
