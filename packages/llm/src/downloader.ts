...
export async function downloadFile(
    ...
    try {
      ...
      if (actualHash !== expectedHash) {
        ...
        if (retriesConsumed < maxRetries) {
          ...
          onStateChange?.('downloading');
          await sleep(exponentialBackoff.delayMs(retriesConsumed), signal);
          const retryResult = await downloadFile({ ...options, maxRetries: 0 }); // recurse once
  ...
  try {
    ...
    return {
      ...
      error: {
        type: 'unknown',
        message: `Failed to move file to destination: ${err instanceof Error ? err.message : String(err)}`,
        bytesReceived: offset,
        retryable: false,
        cause: err,
      },
...
function isAdapterError(
  err: unknown,
): err is { type: DownloadError['type']; message: string; retryable?: boolean; cause?: unknown } {
  return (
    typeof err === 'object' &&
    err !== null &&
    'type' in err &&
