...
export const SCHEMA_DDL: Record<number, string[]> = {
  1: [
    ...
    "CREATE TABLE IF NOT EXISTS syllabus_topic_tree (id TEXT PRIMARY KEY, subject TEXT NOT NULL CHECK(subject IN ('math','english','chinese_mt','science')), level TEXT NOT NULL CHECK(level IN ('P1','P2','P3','P4','P5','P6')), topic_id TEXT NOT NULL, parent_topic_id TEXT, name_en TEXT NOT NULL, name_zh TEXT NOT NULL, sequence INTEGER NOT NULL DEFAULT 0, UNIQUE(subject, level, topic_id))",
    "CREATE TABLE IF NOT EXISTS model_metadata (id TEXT PRIMARY KEY, model_name TEXT NOT NULL, version TEXT NOT NULL, quant TEXT NOT NULL, file_path TEXT NOT NULL, hash TEXT NOT NULL, downloaded_at TEXT NOT NULL, size_bytes INTEGER NOT NULL, UNIQUE(model_name, version))",
    "CREATE TABLE IF NOT EXISTS usage_counter (id TEXT PRIMARY KEY, kid_profile_id TEXT NOT NULL REFERENCES kid_profile(id) ON DELETE CASCADE, date TEXT NOT NULL, photo_count INTEGER NOT NULL DEFAULT 0, question_count INTEGER NOT NULL DEFAULT 0, UNIQUE(kid_profile_id, date))",
