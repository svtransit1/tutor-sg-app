...
- ✅ Unblocks AAAS-44 (homework flow) via `NativeInferenceBridge` adapter

## NativeInferenceBridge (v2, commit 2695415)

Created `NativeInferenceBridge` — adapter wiring `LLMRuntime` into the `@tutor-sg/llm` `InferenceBridge` interface that AAAS-44 consumes:

- Auto-loads correct model tier (E4B/E2B/Qwen) from `InferenceRequest.deviceTier`
- Subject-based model switching: Gemma ↔ Qwen for Chinese MT
- Uses `buildPrompt()` + `parseInferenceResponse()` from `@tutor-sg/llm`
- Bilingual error fallback (EN + zh-Hans) on inference failure
- 9 unit tests

## Test results

- ✅ **30/30** LLM tests pass (21 LLMRuntime + 9 NativeInferenceBridge)
- Pre-existing 32 failures in KidProfileCreateScreen/ParentPinSetupScreen (unrelated)

## Branch

- **Branch:** `wolf/aaas-127-litert-native-module`
- **Current commit:** `2695415`
