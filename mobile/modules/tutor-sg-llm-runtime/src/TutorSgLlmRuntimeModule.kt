  ...
  override fun definition() = ModuleDefinition {
      ...
      try {
        ...
        if (!modelFile.exists()) {
          sendEvent("onError", mapOf(
            ...
            "message" to "Model file not found at $path",
            "recoverable" to true,
          ))
          emitState("error")
          return@AsyncFunction mapOf(
            "success" to false, "modelId" to "", "state" to "error",
            "message" to "Model file not found",
          )
        }
        if (modelFile.length() == 0L) {
          sendEvent("onError", mapOf(
            "code" to "MODEL_EMPTY",
            "message" to "Model file is empty",
            "recoverable" to true,
          ))
          emitState("error")
          return@AsyncFunction mapOf(
    ...
    AsyncFunction("generate") { prompt: String, maxTokens: Int, temperature: Double, topP: Double ->
      if (currentState != "ready") {
        sendEvent("onError", mapOf(
          ...
          "message" to "Call loadModel first. Current state: $currentState",
          "recoverable" to true,
        ))
        return@AsyncFunction mapOf(
          "text" to "", "tokenCount" to 0, "latencyMs" to 0, "tokensPerSecond" to 0,
        )
      }
      try {
        ...
        emitState("ready")
        mapOf(
          "text" to "", "tokenCount" to 0,
          "latencyMs" to (System.currentTimeMillis() - startMs),
          "tokensPerSecond" to 0,
        )
      } catch (e: Exception) {
    ...
    AsyncFunction("unloadModel") {
      try {
        ...
        android.util.Log.e("TutorSgLlmRuntime", "unload failed", e)
      }
    }
