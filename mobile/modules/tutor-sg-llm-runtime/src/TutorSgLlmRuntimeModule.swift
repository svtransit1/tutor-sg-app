...
public class TutorSgLlmRuntimeModule: Module {
  private let inferenceQueue = DispatchQueue(
    label: "com.tutorsg.llm", qos: .userInitiated
  )
  private var modelLoaded = false
  private var loadedModelId: String?
  private var loadedTier: String?
    ...
    AsyncFunction("loadModel") { (path: String, tier: String, promise: Promise) in
      self.inferenceQueue.async {
        ...
        guard FileManager.default.fileExists(atPath: path) else {
          ...
          promise.resolve([
            "success": false, "modelId": "", "state": "error",
            "message": "Model file not found",
          ])
          return
        }
        self.loadedModelId = (path as NSString).lastPathComponent
    ...
    AsyncFunction("generate") { (prompt: String, maxTokens: Int, temperature: Double, topP: Double, promise: Promise) in
      guard self.currentState == "ready" else {
        self.sendEvent("onError", [
          ...
          "message": "Call loadModel first. Current state: \(self.currentState)",
          "recoverable": true,
        ])
        promise.resolve(["text": "", "tokenCount": 0, "latencyMs": 0, "tokensPerSecond": 0])
        return
      }
      self.inferenceQueue.async {
        self.emitState("generating")
        let startMs = CFAbsoluteTimeGetCurrent() * 1000
        let elapsedMs = (CFAbsoluteTimeGetCurrent() * 1000) - startMs
        self.currentState = "ready"
        self.emitState("ready")
        promise.resolve([
          "text": "", "tokenCount": 0,
          "latencyMs": Int(elapsedMs), "tokensPerSecond": 0,
        ])
      }
    }
