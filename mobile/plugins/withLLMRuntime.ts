...
const withLLMRuntime: ConfigPlugin = (config) => {
  config = withDangerousMod(config,['android',async(m)=>{
    const dst=path.resolve(m.modRequest.projectRoot,'android','app','src','main','java','com','aaas','tutorsg','llm');
    fs.mkdirSync(dst,{recursive:true});
    for(const f of['LLMRuntimeModule.java','LLMRuntimePackage.java']){
      const src=path.resolve(__dirname,'..','android','app','src','main','java','com','aaas','tutorsg','llm',f);
      if(fs.existsSync(src))fs.copyFileSync(src,path.join(dst,f));
    }
    return m;
  }]);
  config = withDangerousMod(config,['ios',async(m)=>{
  ...
  config = withPodfileProperties(config,(p)=>({...p,podfileProperties:{...p.podfileProperties,LLM_RUNTIME_ENABLED:'1'}}));
  return config;
};
