  ...
  if (!permission?.granted) {
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.permissionSubtext, { color: isDark ? '#888888' : '#6B7280' }]}>
          {t('cameraScreen.permission.description')}
        </Text>
        <TouchableOpacity
          style={[styles.permissionButton, { backgroundColor: isDark ? '#2563EB' : '#4A90D9' }]}
          onPress={requestPermission}
          accessibilityRole="button"
          accessibilityLabel={t('cameraScreen.permission.cta')}
        >
          <Text style={styles.permissionButtonText}>{t('cameraScreen.permission.cta')}</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={[styles.container, { backgroundColor: '#000' }]}>
      ...
      {processingStage === 'error' && (
        <View style={styles.errorOverlay}>
          <View style={[styles.errorCard, { backgroundColor: isDark ? '#1E1E1E' : '#FFFFFF' }]}>
            <Text style={styles.errorIcon}>😅</Text>
            <Text style={[styles.errorTitle, { color: isDark ? '#FFFFFF' : '#1A1A1A' }]}>
              {t('cameraScreen.error.title')}
            </Text>
            <TouchableOpacity
              style={[styles.retakeButton, { backgroundColor: isDark ? '#2563EB' : '#4A90D9' }]}
              onPress={handleRetake}
              accessibilityRole="button"
            >
              <Text style={styles.retakeButtonText}>{t('cameraScreen.error.retake')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              accessibilityRole="button"
            >
              <Text style={[styles.cancelRetakeText, { color: isDark ? '#90CAF9' : '#6B7280' }]}>
                {t('common.cancel')}
              </Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  // ── Permission ──
  permissionText: { fontSize: 18, fontWeight: '700', marginBottom: 8, textAlign: 'center' },
  ...
  retakeButton: { paddingVertical: 12, paddingHorizontal: 32, borderRadius: 12, marginTop: 8 },
  retakeButtonText: { color: '#FFFFFF', fontSize: 16, fontWeight: '700' },
  cancelRetake: { paddingVertical: 8 },
  cancelRetakeText: { fontSize: 14, fontWeight: '500' },
});
