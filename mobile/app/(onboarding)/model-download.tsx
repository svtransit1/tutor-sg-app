...
export default function ModelDownloadRoute() {
  ...
  const calcETA = useCallback((downloaded: number, speed: number): number | null => {
    if (speed <= 0) return null;
    const remaining = MOCK_TOTAL_BYTES - downloaded;
  ...
  const measureSpeed = useCallback((bytes: number): number => {
    ...
    if (elapsed >= 1) {
      const diff = bytes - lastBytesRef.current;
      if (diff >= 0) {
        speedWindowRef.current.push({ bytes: diff, time: elapsed });
        if (speedWindowRef.current.length > 10) speedWindowRef.current.shift();
      }
      lastBytesRef.current = bytes;
      lastTimeRef.current = now;
    }
    if (speedWindowRef.current.length === 0) return 0;
    const totalBytes = speedWindowRef.current.reduce((sum, s) => sum + s.bytes, 0);
    const totalTime = speedWindowRef.current.reduce((sum, s) => sum + s.time, 0);
  ...
  const startDownload = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      const speed = measureSpeed(downloadedBytes + MOCK_SPEED);
      setSpeedBps(speed);
      if (speed > 0) setShowSpeed(true);
      const eta = calcETA(downloadedBytes + MOCK_SPEED, speed);
      setEtaSeconds(eta);
    }, 100);
  }, [measureSpeed, calcETA, downloadedBytes]);
        ...
        {phase !== 'error' && phase !== 'cellular_warning' && (
          <>
            <View style={styles.progressSection}>
              ...
              {phase === 'downloading' && etaSeconds !== null && (
                <Text style={styles.etaText}>
                  {t('modelDownload.etaTime', {
                    eta: formatETA(etaSeconds),
                  })}
                  {showSpeed && speedBps > 0 && ` · ${t('modelDownload.speed', { speed: formatSpeed(speedBps) })}`}
                </Text>
              )}
            </View>
