const path = require('path');
const rnDir = path.dirname(require.resolve('react-native'));

module.exports = {
  preset: 'react-native',
  rootDir: '.',
  roots: ['<rootDir>'],
  testMatch: ['**/__tests__/**/*.test.(ts|tsx|js)'],
  testPathIgnorePatterns: [
    '<rootDir>/src/home/',
  ],
  transformIgnorePatterns: [
    'node_modules/(?!(jest-)?react-native|@react-native(-community)?|expo(-.*)?|@expo(-.*)?|react-native-.*)/',
  ],
  transform: {
    '^.+\\.(js|ts|tsx)$': ['babel-jest', { configFile: './babel.config.js' }],
    '^.+\\.(bmp|gif|jpg|jpeg|mp4|png|psd|svg|webp)$': path.join(
      rnDir,
      'jest',
      'assetFileTransformer.js',
    ),
  },
  moduleNameMapper: {
    '^@/(.*)$': '<rootDir>/src/$1',
    '^uuid$': '<rootDir>/__mocks__/uuid.ts',
    '^react-native-mmkv$': '<rootDir>/__mocks__/react-native-mmkv.ts',
    '^react-native-worklets$': '<rootDir>/__mocks__/react-native-worklets.ts',
    '^i18next$': '<rootDir>/__mocks__/i18next.ts',
    '^react-i18next$': '<rootDir>/__mocks__/react-i18next.ts',
    '^@react-native-community/netinfo$': '<rootDir>/__mocks__/@react-native-community/netinfo.ts',
    '^@react-native-async-storage/async-storage$': '<rootDir>/__mocks__/async-storage.ts',
    '^expo-sqlite$': '<rootDir>/__mocks__/expo-sqlite.ts',
    '^expo-file-system$': '<rootDir>/__mocks__/expo-file-system.ts',
    '^expo-speech$': '<rootDir>/__mocks__/expo-speech.ts',
    '^expo-crypto$': '<rootDir>/__mocks__/expo-crypto.ts',
    '^expo-camera$': '<rootDir>/__mocks__/expo-camera.ts',
    '^expo-constants$': '<rootDir>/__mocks__/expo-constants.ts',
    '^expo-localization$': '<rootDir>/__mocks__/expo-localization.ts',
    '^expo-modules-core$': '<rootDir>/__mocks__/expo-modules-core.ts',
    '^expo-secure-store$': '<rootDir>/__mocks__/expo-secure-store.ts',
    '^expo-web-browser$': '<rootDir>/__mocks__/expo-web-browser.ts',
    '^expo-auth-session$': '<rootDir>/__mocks__/expo-auth-session.ts',
    '^expo-linking$': '<rootDir>/__mocks__/expo-linking.ts',
    '^expo-router$': '<rootDir>/__mocks__/expo-router.tsx',
    '^react-native-safe-area-context$': '<rootDir>/__mocks__/react-native-safe-area-context.ts',
    '^@supabase/supabase-js$': '<rootDir>/__mocks__/@supabase/supabase-js.ts',
    '^@tutor-sg/database$': '<rootDir>/../packages/database/src',
    '^@tutor-sg/database/(.*)$': '<rootDir>/../packages/database/src/$1',
    '^@tutor-sg/device-tier$': '<rootDir>/__mocks__/@tutor-sg/device-tier.tsx',
    '^@tutor-sg/device-tier/(.*)$': '<rootDir>/__mocks__/@tutor-sg/device-tier.tsx',
    '^@tutor-sg/features$': '<rootDir>/../packages/features/src',
    '^@tutor-sg/features/(.*)$': '<rootDir>/../packages/features/src/$1',
    '^@tutor-sg/i18n$': '<rootDir>/../packages/i18n/src',
    '^@tutor-sg/i18n/(.*)$': '<rootDir>/../packages/i18n/src/$1',
    '^@tutor-sg/llm$': '<rootDir>/__mocks__/@tutor-sg/llm.ts',
    '^@tutor-sg/llm/(.*)$': '<rootDir>/__mocks__/@tutor-sg/llm.ts',
    '^@tutor-sg/perf$': '<rootDir>/../packages/perf/src',
    '^@tutor-sg/perf/(.*)$': '<rootDir>/../packages/perf/src/$1',
    '^@tutor-sg/shared$': '<rootDir>/../packages/shared/src',
    '^@tutor-sg/shared/(.*)$': '<rootDir>/../packages/shared/src/$1',
    '^expo-(.*)$': '<rootDir>/__mocks__/expo-module-proxy.ts',
  },
  setupFiles: [path.join(rnDir, 'jest', 'setup.js')],
  setupFilesAfterEnv: ['<rootDir>/setup-jest.ts'],
  clearMocks: true,
  collectCoverage: true,
  collectCoverageFrom: [
    'src/**/*.(ts|tsx)',
    '!src/**/*.types.ts',
    '!src/**/__tests__/**',
    '!src/i18n/**',
    '!src/components/**',
    '!src/services/**',
  ],
  coverageThreshold: { global: { lines: 0, branches: 0, functions: 0 } },
  testTimeout: 30000,
};
