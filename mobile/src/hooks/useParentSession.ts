...
export interface UseParentSessionReturn {
  ...
  logQuestionAttempt: (questionId: string, correct: boolean, hintsUsed: number, timeSeconds: number, struggleDetected: boolean) => Promise<void>
...
export function useParentSession(): UseParentSessionReturn {
  const startParentSession = useCallback(async (subject: ParentSubject, topic = ''): Promise<string | null> => {
    try {
