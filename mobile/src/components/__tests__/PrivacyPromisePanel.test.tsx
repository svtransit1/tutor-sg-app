...
const mockT = jest.fn((key: string) => {
  const FALLBACKS: Record<string, string> = {
    ...
    'onboarding.privacyPromise.badgeText': "Your child's data never leaves this device",
...
describe('PrivacyPromisePanel', () => {
  ...
  describe('card variant (default)', () => {
    ...
    it('renders the headline from i18n', () => {
      render(<PrivacyPromisePanel />)
      expect(screen.getByText('Your privacy is our promise')).toBeTruthy()
    })
    ...
    it('expands and collapses the learn-more section', () => {
      ...
      // Tap to expand
      fireEvent.press(screen.getByLabelText('Expand to learn more about privacy'))
      expect(
        screen.getByText(
          'All AI processing happens right on your device using a small but powerful AI model. No cloud servers, no uploads, no data collection.',
        ),
      ...
      // Tap to collapse
      fireEvent.press(screen.getByLabelText('Collapse privacy details'))
      expect(
        screen.queryByText(
          'All AI processing happens right on your device using a small but powerful AI model. No cloud servers, no uploads, no data collection.',
        ),
    ...
    it('has accessibility summary role', () => {
      render(<PrivacyPromisePanel />)
      expect(
        screen.getByLabelText("Privacy promise: your child's data never leaves this device."),
      ).toBeTruthy()
    })
  ...
  describe('badge variant', () => {
    ...
    it('renders the badge text from i18n', () => {
      render(<PrivacyPromisePanel variant="badge" />)
      expect(screen.getByText("Your child's data never leaves this device")).toBeTruthy()
    })

    it('renders overridden body when body prop is provided', () => {
      render(<PrivacyPromisePanel variant="badge" body="Custom badge text" />)
  ...
  describe('inline variant', () => {
    ...
    it('renders overridden body when body prop is provided', () => {
      render(<PrivacyPromisePanel variant="inline" body="Custom inline text" />)
  ...
  it('accepts a custom style prop', () => {
    render(<PrivacyPromisePanel style={{ marginTop: 20, maxWidth: 300 }} />)
    expect(screen.getByText('🛡️')).toBeTruthy()
  })
