 ...
 * Key: `download.state.<fileName>` → JSON(serialized DownloadState)
 * Key: `download.completed` → JSON(string[] of downloaded file names)
 */
...
function store(): MMKV {
  if (!_store) {
    _store = new MMKV({ id: 'model_download' });
  }
  return _store;
}
...
export function markModelDownloaded(fileName: string): void {
  ...
  if (!completed.includes(fileName)) {
    ...
    store().set(COMPLETED_KEY, JSON.stringify(completed));
  }
