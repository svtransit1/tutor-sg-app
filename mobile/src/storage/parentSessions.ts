...
function rowToSession(row: ParentSessionRow): ParentSession {
  return {
    ...
    subject: row.subject as ParentSubject,
    topic: row.topic,
...
export class ParentSessionRepository {
  static async startSession(
    ...
    await db.runAsync(
      "INSERT INTO parent_sessions (id, kid_profile_id, subject, topic, started_at) VALUES (?, ?, ?, ?, datetime('now'))",
      id,
      kidProfileId,
      subject,
      topic,
  ...
  static async logQuestionAttempt(
    ...
    const db = await getDb();
    await db.runAsync(
      'INSERT INTO question_attempts (session_id, question_id, correct, hints_used, time_seconds, struggle_detected) VALUES (?, ?, ?, ?, ?, ?)',
      sessionId,
      questionId,
      correct ? 1 : 0,
      hintsUsed,
      timeSeconds,
      struggleDetected ? 1 : 0,
    );
    await db.runAsync(
      'UPDATE parent_sessions SET questions_attempted = questions_attempted + 1, questions_correct = COALESCE(questions_correct, 0) + ?, struggle_indicators = struggle_indicators || ? WHERE id = ?',
      correct ? 1 : 0,
      struggleDetected ? '1' : '0',
      sessionId,
    );
  ...
  static async endSession(
    ...
    const db = await getDb();
    await db.runAsync(
      "UPDATE parent_sessions SET ended_at = datetime('now'), ai_summary = ?, parent_flagged = ? WHERE id = ?",
      aiSummary,
      parentFlagged ? 1 : 0,
      sessionId,
    );
  ...
  static async getSession(id: string): Promise<ParentSession | null> {
    const db = await getDb();
    const row = await db.getFirstAsync<ParentSessionRow>(
      'SELECT * FROM parent_sessions WHERE id = ?',
      id,
    );
  ...
  static async getSessionsForKid(kidProfileId: string): Promise<ParentSession[]> {
    const db = await getDb();
    const rows = await db.getAllAsync<ParentSessionRow>(
      'SELECT * FROM parent_sessions WHERE kid_profile_id = ? ORDER BY started_at ASC',
      kidProfileId,
    );
