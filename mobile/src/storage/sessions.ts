 ...
 * The inference_result column stores the full InferenceResponse as JSON.
 */
    ...
    dbPromise = openDatabaseAsync(DB_NAME).then(async (db) => {
      await db.execAsync(`
        CREATE TABLE IF NOT EXISTS kid_sessions (
          ...
          subject TEXT NOT NULL CHECK(subject IN ('math','english','science','chinese','chinese_mt')),
...
export async function insertFullSession(session: {
  ...
  const subject = (['math', 'english', 'science', 'chinese', 'chinese_mt'].includes(session.subject)
    ? session.subject
    : 'math') as KidSession['subject'];
  await db.runAsync(
    'INSERT INTO kid_sessions (subject, question_count, created_at) VALUES (?, ?, ?)',
    subject,
    session.questionCount,
 ...
 */
export async function getSessionById(sessionId: string): Promise<FullHomeworkSession | null> {
  try {
    const db = await getDb();
 ...
 */
export async function getAllHomeworkSessions(limit = 50): Promise<FullHomeworkSession[]> {
  const db = await getDb();
  const rows = await db.getAllAsync<{
    id: number;
    session_id: string;
