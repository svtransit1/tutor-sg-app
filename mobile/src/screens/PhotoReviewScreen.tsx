...
export function parseQuestions(raw: string | string[] | undefined): QuestionFeedback[] | null {
  if (!raw) return null;
  try { const p = JSON.parse(Array.isArray(raw) ? raw[0] : raw); return Array.isArray(p) ? p as QuestionFeedback[] : null; }
  catch { return null; }
}

function mockQuestions(): QuestionFeedback[] {
  return [
    { questionNumber: 1, subject: 'math', topic: 'Addition', questionText: 'John has 15 apples. He buys 7 more. How many apples does he have now?', scaffoldedHelp: { hint: 'Think about what the problem is asking.', guidedSteps: ['Step 1: John starts with 15 apples.', 'Step 2: He buys 7 more, so add.', 'Step 3: 15 + 7 = ?'], workedSolution: '15 + 7 = 22. John has 22 apples.' } },
    { questionNumber: 2, subject: 'math', topic: 'Subtraction', questionText: 'Sarah had 30 stickers. She gave 12 away. How many left?', scaffoldedHelp: { hint: 'When someone gives away items, subtract.', guidedSteps: ['Step 1: Starts with 30.', 'Step 2: Gives 12 away.', 'Step 3: 30 - 12 = ?'], workedSolution: '30 - 12 = 18.' } },
  ];
}
...
export default function PhotoReviewScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (parsed && parsed.length > 0) { setResult({ sessionId: 'session-' + Date.now(), questions: parsed }); setPhase('ready'); }
      else { setResult({ sessionId: 'mock-session', questions: mockQuestions() }); setPhase('ready'); }
    }, 1200);
  ...
  return (
    <View style={[st.root, { backgroundColor: bgColor }]}>
      <View style={st.topBar}>
        <TouchableOpacity onPress={goBack} accessibilityRole="button" accessibilityLabel={t('common.back')} style={st.backBtn}>
          <Text style={[st.backArrow, { color: isDark ? '#90CAF9' : '#2563EB' }]}>←</Text>
          <Text style={[st.backLabel, { color: isDark ? '#90CAF9' : '#2563EB' }]}>{t('common.back')}</Text>
        </TouchableOpacity>
        <Text style={[st.heading, { color: textColor }]}>{t('photoReview.title')}</Text>
        <View style={st.spacer} />
      </View>
      {phase === 'processing' && <View style={st.skelList}>{[1, 2].map(i => <HomeworkFeedbackCard key={i} variant="loading" questionNumber={i} />)}</View>}
      {phase === 'empty' && (
      ...
      {phase === 'ready' && result && (
        <ScrollView style={st.scroll} contentContainerStyle={st.scrollCont} accessibilityRole="list" accessibilityLabel={t('photoReview.accessibility.questionList')}>
          {result.questions.map(q => <HomeworkFeedbackCard key={q.questionNumber} help={q.scaffoldedHelp} questionNumber={q.questionNumber} questionText={q.questionText} title={t('homeworkFeedback.title')} style={{ marginBottom: 16 }} />)}
        </ScrollView>
      )}
    </View>
  );
