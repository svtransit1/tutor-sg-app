...
const TIER_DISPLAY: Record<DeviceTier, { labelKey: string; descKey: string; size: string }> = {
  high: {
    ...
    descKey: 'deviceTierResult.highDescription',
    size: '~4.5 GB',
  },
  mid: {
    labelKey: 'deviceTierResult.standard',
    descKey: 'deviceTierResult.standardDescription',
    size: '~1.7 GB',
  },
  low: {
    labelKey: 'deviceTierResult.standard',
 ...
 */
const networkTypeOverride: NetworkType | null = null
    ...
    return (
      <View style={[styles.badge, high ? styles.badgeHigh : styles.badgeStd]}>
        ...
        <View style={styles.badgeText}>
          <Text style={[styles.badgeLabel, high ? styles.labelHigh : styles.labelStd]}>
            {t(info.labelKey)}
          </Text>
          <Text style={[styles.badgeDesc, high ? styles.descHigh : styles.descStd]}>
            {t(info.descKey)}
          </Text>
        </View>
      </View>
        ...
        <View style={styles.cardRow}>
          ...
          <Text style={styles.cardLabel}>
            {t('deviceTierResult.modelNames', {
              models: models ? `${models.llm} + ${models.mt}` : '',
            })}
          </Text>
        </View>
        <View style={styles.cardRow}>
          <Text style={styles.cardIcon}>📦</Text>
          <Text style={styles.cardLabel}>{t('deviceTierResult.downloadSize', { size })}</Text>
        </View>
        <View style={[styles.cardRow, styles.cardRowLast]}>
          <Text style={styles.cardIcon}>🔒</Text>
          <Text style={styles.cardConsent}>{t('deviceTierResult.consentText')}</Text>
        </View>
      </View>
    )
  }
