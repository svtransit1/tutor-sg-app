...
const mockT = jest.fn((key: string) => {
  const FALLBACKS: Record<string, string> = {
    'onboarding.privacyPromise.headline':
      'Before we start — a quick promise about privacy',
    'onboarding.privacyPromise.continue':
    ...
    'onboarding.privacyPromise.learnMoreBody':
      'All AI processing happens right on your device.',
...
describe('PrivacyPromiseScreen', () => {
  ...
  it('renders the continue button', () => {
    render(<PrivacyPromiseScreen />);
    expect(
      screen.getByText('I understand — continue'),
    ).toBeTruthy();
  });
  ...
  it('fires onContinue when continue button is pressed', () => {
    ...
    fireEvent.press(
      screen.getByLabelText('I understand — continue'),
    );
