...
function KBtn({
  ...
  return (
    <Pressable
      style={({ pressed }) => [
        s.keypadBtn,
        pressed && s.keypadBtnPressed,
        disabled && s.keypadBtnDisabled,
      ]}
      onPress={() => onPress(v)}
      disabled={disabled}
    ...
    >
      <Text
        style={[
          s.keypadBtnText,
          v === '\u232B' && s.keypadBackspace,
          disabled && s.keypadBtnTextDisabled,
        ]}
      >
        {v}
      </Text>
      ...
      if (np.length === PIN_LENGTH) {
        if (await verifyPin(np)) {
          ...
          if (r <= 0) {
            ...
            setAttemptsLeft(r);
            setError(t('parentAuth.wrongPin', { attempts: r }));
          }
          setPin('');
        }
      }
        ...
        <View style={s.content}>
          ...
          <Text style={s.attemptsLabel}>
            {attemptsLeft > 0 && attemptsLeft <= 3
              ? t('parentAuth.attemptsRemaining', { count: attemptsLeft })
              : ''}
          </Text>
          {cooldown > 0 ? (
            <View style={s.cooldownContainer}>
              <Text style={s.cooldownText} accessibilityRole="alert">
                {t('parentAuth.cooldown', { seconds: cooldown })}
              </Text>
            </View>
          ) : (
            <>
              ...
              <View style={s.pinRow}>
                {Array.from({ length: PIN_LENGTH }).map((_, i) => (
                  <Slot key={i} filled={i < pin.length} i={i} />
                ))}
              </View>
              <View style={s.keypad}>
                {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((d) => (
                  <KBtn key={d} v={d} onPress={kp} />
                ))}
          ...
          <Pressable style={s.cancelBtn} onPress={dismiss} accessibilityRole="button">
            <Text style={s.cancelBtnText}>{t('parentAuth.cancel')}</Text>
          </Pressable>
        </View>
      </View>
    </View>
