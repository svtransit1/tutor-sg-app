...
function DigitSlot({ filled, i }: { filled: boolean; i: number }) {
  return (
    <View
      style={[s.digitSlot, filled && s.digitSlotFilled]}
      accessibilityLabel={filled ? `Digit ${i + 1} entered` : `Digit ${i + 1}`}
    >
...
function KeyBtn({
  ...
  return (
    <Pressable
      style={({ pressed }) => [s.keypadBtn, pressed && s.keypadBtnPressed]}
      onPress={() => onPress(v)}
      disabled={disabled}
      accessibilityRole="button"
      accessibilityLabel={`Key ${v}`}
    >
...
export default function ParentPinSetupScreen({
  ...
  const goBack = useCallback(() => {
    ...
    } else if (phase === 'enter' && mode === 'change') {
      setPhase('old');
      setPin('');
    }
  }, [phase, pin, mode]);
  ...
  const title =
    mode === 'change' ? t('parentAuth.changePin.title') : t('onboarding.parentPinSetup.title');
  ...
  if (phase === 'old') stepLabel = t('parentAuth.changePin.enterOld');
    ...
    stepLabel =
      mode === 'change'
        ? t('parentAuth.changePin.enterNew')
      ...
      <View style={s.content}>
        ...
        {(mismatch || wrongOld) && (
          <Text style={s.errorText} accessibilityRole="alert">
            {mismatch
              ? t('onboarding.parentPinSetup.mismatch')
              : t('parentAuth.changePin.wrongOld')}
          </Text>
        )}
        <View style={[s.pinRow, (mismatch || wrongOld) && s.pinRowError]}>
          {Array.from({ length: PIN_LENGTH }).map((_, i) => (
            <DigitSlot key={i} filled={i < cur.length} i={i} />
          ))}
        </View>
        <View style={s.keypad}>
          {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((d) => (
            <KeyBtn key={d} v={d} onPress={kp} disabled={saving} />
          ))}
        ...
        {(phase === 'confirm' || (phase === 'enter' && mode === 'change')) && (
          <Pressable style={s.backLink} onPress={goBack} accessibilityRole="button">
        ...
        {mode === 'change' && onCancel && (
          <Pressable style={s.skipLink} onPress={onCancel} accessibilityRole="button">
            <Text style={s.skipLinkText}>{t('parentAuth.cancel')}</Text>
          </Pressable>
        )}
      </View>
    </View>
