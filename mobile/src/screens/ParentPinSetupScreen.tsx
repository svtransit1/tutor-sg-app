  ...
  return (
    <View style={s.container}><View style={s.content}>
      ...
      {(phase === 'confirm' || (phase === 'enter' && mode === 'change')) && (
        <Pressable style={s.backLink} onPress={goBack} accessibilityRole="button" accessibilityLabel={t('common.back')}><Text style={s.backLinkText}>{'\u2190'} {t('common.back')}</Text></Pressable>
      )}
      {mode === 'setup' && onSkip && (
      ...
      {mode === 'change' && onCancel && (
        <Pressable style={s.skipLink} onPress={onCancel} accessibilityRole="button" accessibilityLabel={t('parentAuth.cancel')}><Text style={s.skipLinkText}>{t('parentAuth.cancel')}</Text></Pressable>
      )}
    </View></View>
  );
}
