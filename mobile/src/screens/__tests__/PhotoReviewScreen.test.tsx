  ...
  useTranslation: () => {
    const t = (key: string, opts?: Record<string, unknown>) => {
      const keys: Record<string, string> = {
        'kidHome.camera.photoReview.retake': 'Retake',
        'kidHome.camera.photoReview.usePhoto': 'Use this photo',
        'kidHome.camera.photoReview.retakeButton': 'Retake the photo',
        'kidHome.camera.photoReview.confirmButton': 'Use this photo and continue',
        'kidHome.camera.photoReview.addPageButton': 'Capture another page',
        'kidHome.camera.photoReview.addPage': 'Add page',
        'kidHome.camera.photoReview.removePage': 'Remove',
        'kidHome.camera.photoReview.multiPageHint': 'You can add multiple pages',
        'kidHome.camera.photoReview.crop': 'Crop',
        'kidHome.camera.photoReview.cropButton': 'Adjust crop area',
        'kidHome.camera.photoReview.cancelCrop': 'Cancel',
        'kidHome.camera.photoReview.confirmCrop': 'Confirm crop',
      };
      if (opts && opts.count && key === 'kidHome.camera.photoReview.usePhotos') {
        return `Use ${opts.count} photos`;
      }
      if (opts && opts.count && key === 'kidHome.camera.photoReview.usePhotos_plural') {
        return `Use ${opts.count} photos`;
      }
      if (opts && opts.count && key === 'kidHome.camera.photoReview.confirmButtonPlural') {
        return `Use all ${opts.count} photos and continue`;
      }
      if (opts && opts.number && key === 'kidHome.camera.photoReview.pageThumbnail') {
        return `Page ${opts.number} thumbnail`;
      }
      if (opts && opts.number && key === 'kidHome.camera.photoReview.removePageButton') {
        return `Remove page ${opts.number}`;
      }
      if (opts && opts.current && opts.total && key === 'kidHome.camera.photoReview.pageCount') {
...
describe('PhotoReviewScreen', () => {
  ...
  describe('bilingual support', () => {
    it('has all i18n keys defined in both EN and zh-Hans', () => {
      ...
      const enKeys = en?.kidHome?.camera?.photoReview;
      const zhKeys = zh?.kidHome?.camera?.photoReview;
