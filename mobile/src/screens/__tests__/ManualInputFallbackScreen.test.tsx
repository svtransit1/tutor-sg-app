...
describe('ManualInputFallbackScreen', () => {
  ...
  it('shows parse error state when items param is invalid', () => {
    ...
    expect(screen.getByText('cameraScreen.error.title')).toBeTruthy()
  })
  ...
  it('shows TextInput fields in type mode', async () => {
    ...
    const placeholders = await waitFor(() =>
      screen.getAllByPlaceholderText('manualInputFallback.typePlaceholder'),
    )
  ...
  it('shows submit button with remaining count', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('manualInputFallback.submit (2)')).toBeTruthy()
    })
  })
  ...
  it('validates at least one answer before submitting', async () => {
    ...
    const submitButton = await waitFor(() => screen.getByText('manualInputFallback.submit (2)'))
  ...
  it('navigates back to camera on submit when an answer is provided', async () => {
    ...
    const submitButton = await waitFor(() => screen.getByText('manualInputFallback.submit (1)'))
  ...
  it('includes captured page URIs when navigating back on submit', async () => {
    mockSearchParams({
      items: VALID_ITEMS,
      capturedPageUris: JSON.stringify(['file://photo1.jpg', 'file://photo2.jpg']),
    })
  ...
  it('has accessibility label on skip buttons', () => {
    render(<ManualInputFallbackScreen />)
    const skipButtons = screen.getAllByLabelText('manualInputFallback.accessibility.skip')
