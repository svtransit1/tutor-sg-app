...
// ── Required funnel events (AAAS-47 acceptance criteria) ─────────

describe('required funnel events', () => {
  it('tracks consent_given', async () => {
    const svc = createService();
    await svc.setOptIn(true);
    await svc.track('consent_given', { consent_method: 'parent_gate' });
    expect(svc.lastEvent()!.name).toBe('consent_given');
    expect(svc.lastEvent()!.properties.consent_method).toBe('parent_gate');
  });

  it('tracks first_homework_submitted', async () => {
    const svc = createService();
    await svc.setOptIn(true);
    await svc.track('first_homework_submitted', { subject: 'math', page_count: 1 });
    expect(svc.lastEvent()!.name).toBe('first_homework_submitted');
    expect(svc.lastEvent()!.properties.subject).toBe('math');
    expect(svc.lastEvent()!.properties.page_count).toBe(1);
  });

  it('tracks first_feedback_received', async () => {
    const svc = createService();
    await svc.setOptIn(true);
    await svc.track('first_feedback_received', { subject: 'math', feedback_type: 'hint' });
    expect(svc.lastEvent()!.name).toBe('first_feedback_received');
    expect(svc.lastEvent()!.properties.subject).toBe('math');
    expect(svc.lastEvent()!.properties.feedback_type).toBe('hint');
  });

  it('no PII in any required funnel event', async () => {
    const svc = createService();
    await svc.setOptIn(true);
    await svc.track('consent_given', { consent_method: 'parent_gate' });
    await svc.track('first_homework_submitted', { subject: 'math', page_count: 1 });
    ...
    const piiKeys = ['name', 'kid_name', 'child_name', 'photo', 'ocr', 'free_text', 'answer'];
    for (const evt of svc.events) {
      const keys = Object.keys(evt.properties);
      for (const pii of piiKeys) {
        expect(keys.indexOf(pii)).toBe(-1);
      }
    }
  });
});
